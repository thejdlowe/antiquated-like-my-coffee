import Bryn from './Bryn.png';
import Chelsea from './Chelsea.png';
import Indy from './Indy.png';
import JD from './JD.png';
import Justin from './Justin.png';
import Kim from './Kim.png';
import Lindy from './Lindy.png';
import Lisa from './Lisa.png';
import Melissa from './Melissa.png';
import Michelle from './Michelle.png';

import Logo from './logo.png';

export const playerImages = {
	Bryn,
	Chelsea,
	Indy,
	JD,
	Justin,
	Kim,
	Lindy,
	Lisa,
	Melissa,
	Michelle,
};

export const logo = Logo;

//amongus: [0, 3356],
// 		fortnite: [6468, 6557 - 6468],
// 		gameboy: [7556, 8962 - 7556],
// 		homerun: [10275, 11372 - 10275],
// 		icq: [12948, 14081 - 12948],
// 		mariopause: [17188, 18185 - 17188],
// 		metalgear: [19859, 21583 - 19859],
// 		mariopowerup: [22427, 23697 - 22427],
// 		konami: [24445, 25216 - 24445],
// 		wheeloffortune: [26008, 27233 - 26008],
// 		zelda: [29336, 31150 - 29336],
export const audioData: [number, number, number][] = [
	[0, 1045, 1045],
	[1487, 2750, 2750 - 1487],
	[3235, 4566, 4566 - 3235],
	[5625, 6211, 6211 - 5625],
	[7056, 8235, 8235 - 7056],
	[8783, 9735, 9735 - 8783],
	[10558, 11634, 11634 - 10558],
	[12089, 12975, 12975 - 12089],
	[13080, 14274, 14274 - 13080],
	[16473, 17584, 17584 - 16473],
	// [0, 3356], //amongus
	// [6468, 6557], //fortnite
	// [7556, 8962], //gameboy
	// [10275, 11372], //homerun
	// [12948, 14081], //icq
	// [17188, 18185], //mariopause
	// [19859, 21583], //metalgear
	// [22427, 23697], //mariopowerup
	// [24445, 25216], //konami
	// [26008, 27233], //wheeloffortune
	// [29336, 31150], //zelda
];
